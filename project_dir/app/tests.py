...
class Anonymous_User_Tests_Cases(APITestCase):
    ...
    def test_anonimous_posts(self):
        url = 'http://127.0.0.1:8000/api/posts_anonymous/'
        response = self.client.get(url, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['previous'], None)
    ...
    def test_create_account(self):
        url = 'http://127.0.0.1:8000/auth/users/'
        data = {'username': 'test_username', 'email': 'manytest@app.com', 'password': 'supersecrettestic'}
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(Person.objects.get(username='test_username').username, 'test_username')
        self.assertEqual(Person.objects.get(username='test_username').email, 'manytest@app.com')
